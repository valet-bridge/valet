...
# Parse the BoardDetails files and make the bids.txt file.
# Very similar to parse.pl, but output the bids and not the 
# results.
...
sub read_details_file
{
  ...
  while (my $line = <$fh>)
  {
    ...
    if ($qboard_flag || $line =~ /^\<\/table\>/)
    {
      ...
      if ($#bid_sequences > 1)
      {
        die "Too many bidding sequences?\n";
      }
      ...
      if ($#bid_sequences == 0)
      {
        # Bit of a kludge.  The bid sequence will be right once.
        print_bids($front1, $entries[0], \@{$bid_sequences[0]});
        print_bids($front2, $entries[6], \@{$bid_sequences[0]});
      }
      else
      {
        print_bids($front1, $entries[0], \@{$bid_sequences[0]});
        print_bids($front2, $entries[6], \@{$bid_sequences[1]});
      }
    ...
    if ($line =~ /\<table\>/ &&
        ...
        $line =~ /West/)
    {
      # Make a new entry in bid_sequences.
      my $newb = 1 + $#bid_sequences;
      fill_row($line, \@{$bid_sequences[$newb]});
    }
...
sub fill_row
{
  ...
  if ($#a <= 7)
  {
    die "Doesn't look like a bidding sequence\n";
  }

  die "Expected West" unless $a[1] =~ /West/;
  die "Expected North" unless $a[2] =~ /North/;
  die "Expected East" unless $a[3] =~ /East/;
  die "Expected South" unless $a[4] =~ /South/;
