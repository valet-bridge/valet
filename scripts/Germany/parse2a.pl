...
# $roundno += 10 if $league > 1;
  ...
  while ($line = <$fh>)
  {
    ...
    {
      warn "New datum score ($lcount): '$line'\n";
      last;
    }

    next unless ($line =~ /TITLE/ ||
      (length($line) > 200 && $line =~ /html/ &&
       $line =~ /^<tr align=right>/));
    ...
    if ($na == 9 && $a[4] =~ /^SA/ && $a[4] ne 'SA')
    {
      splice @a, 5, 0, substr($a[4], 2);
      $a[4] = 'SA';
      $na = $#a;
    }
